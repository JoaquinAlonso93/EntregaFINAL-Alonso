...
function agregarAlCarrito(producto) {
    console.trace('Entrando en agregarAlCarrito');
    //Reviso si el producto está en el carrito.
    let memoria = JSON.parse(localStorage.getItem("licores"));
    let cantidadProductoFinal;
    //Si no hay localstorage lo creo
    if (!memoria || memoria.length === 0) {
        const nuevoProducto = getNuevoProductoParaMemoria(producto)
        localStorage.setItem("licores", JSON.stringify([nuevoProducto]));
        ...
        localStorage.setItem("licores", JSON.stringify(nuevaMemoria));
        actualizarNumeroCarrito();
...
function restarAlCarrito(producto) {
    console.trace('Entrando en restarAlCarrito');
    let memoria = JSON.parse(localStorage.getItem("licores"));
...
function actualizarNumeroCarrito() {
    let cuenta = 0;
    const memoria = JSON.parse(localStorage.getItem("licores"));
    if (memoria && memoria.length > 0) {
